...
##########################
# DynamoDB: chatbot-users (for user data storage)
##########################
resource "aws_dynamodb_table" "chatbot_users" {
  name         = "chatbot-users"
  billing_mode = "PAY_PER_REQUEST"

  # Primary key schema: pk (user ID) + sk (data type)
  hash_key  = "pk"
  range_key = "sk"
  ...
  tags = {
    Project = "AI-Chatbot"
    Purpose = "Store user-submitted data"
    Env     = "local"
  }
}
